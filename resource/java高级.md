...
JVM：jvm内存（执行时数据区）、执行引擎、类加载区。

JVM内存：堆、栈、方法区。

用法：

堆：主要存放被创建的对象。堆还是垃圾收集器管理的主要区域：内存不连续。所以垃圾清理时需要算法。垃圾收集器主要用分代收集的算法，就把堆划分为两个代，年轻代和老年代，再细分就是，Eden和survivor和Tenured。

栈：主要存放基础类型变量和对象的引用。占用内存是连续的，栈用完引发stackoverflowError。

方法区：静态池和常量区，用于存放静态变量和常量以及被JVM加载后的类信息。
